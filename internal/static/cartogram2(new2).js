        ...
        if(version.labels !== null) {
            ...
            */
            ...
            if (version.world) {
                ...
                const x2Gall = project.transformLatitude;
                const y2Gall = project.transformLongitude;
                ...
                // TESTING gall peters -> display svg
                const xMinGall = project.transformLongitude(-180);
                const yMaxGall = project.transformLatitude(90);
                const gallWidth = project.transformLongitude(180) - xMinGall;
                const gallHeight = yMaxGall - project.transformLatitude(-90);
                const scaleX = this.width / gallWidth;
                const scaleY = this.height / gallHeight;
                const x2SVG = (x) => (x - xMinGall) * scaleX;
                const y2SVG = (y) => (yMaxGall - y) * scaleY;
                ...
                var textLabels = text.attr('x', d => xPipeline(d.x))
                    .attr('y', d => yPipeLine(d.y))
                ...
                var labelLines = lines.attr('x1', d => xPipeline(d.x1))
                    .attr('x2', d => xPipeline(d.x2))
                    .attr('y1', d => yPipeLine(d.y1))
                    .attr('y2', d => yPipeLine(d.y2))
                    .attr('stroke-width', 1)
                    .attr('stroke', '#000');
                ...
                // Update value of scale
                // labels.scale_x = gallScale;
                // labels.scale_y = gallScale;

                // Transform the label coordinates
                // for (let i = 0; i < labels.labels.length; i++) {
                //     labels.labels[i].x = xPipeline(labels.labels[i].x);
                //     labels.labels[i].y = yPipeLine(labels.labels[i].y);
                // }
            } else {

                // End of label transformation
